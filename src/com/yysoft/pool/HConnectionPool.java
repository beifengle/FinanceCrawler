...
public class HConnectionPool {
	private static Logger logger = LogManager.getLogger(HConnectionPool.class.getName());
	private String testTable = ""; // 测试连接是否可用的测试表名，默认没有测试表
	private int initialConnections = Constant.INITCONNSIZE; // 连接池的初始大小
	private int incrementalConnections = Constant.INCRECONNSIZE;// 连接池自动增加的大小
	private int maxConnections = Constant.MAXCONNS;  // 连接池最大的大小
	private static Vector connections = null; // 存放连接池中数据库连接的向量 , 初始时为 null
	private String dburl = "jdbc:mysql://" + Constant.DBHOST + ":3306/" 
				+ Constant.DCDB + "?useUnicode=true&amp;characterEncoding="
				+ Constant.UNICODE;
	 ...
	 * @return  
	 */
	public static  int getCurrentConnectionsSize() {
	...
	public synchronized void createPool() throws Exception {
		...
		Driver driver = (Driver) (Class.forName(Constant.DRIVER).newInstance());
	...
	private Connection newConnection() throws SQLException {
		// 创建一个数据库连接
		Connection conn = DriverManager.getConnection(dburl, Constant.USERNAME,
				Constant.PASSWORD);
	...
	public synchronized Connection getConnection() throws SQLException {
		...
		while (conn == null) {
			// 等一会再试
			logger.info("Wait for a free H.connection....");
			logger.info("H.connections:" + connections.size());
			wait(250);
			conn = getFreeConnection(); // 重新再试，直到获得可用的连接，如果
			// getFreeConnection() 返回的为 null
			// 则表明创建一批连接后也不可获得可用连接
		}
//		logger.info("返回,H.connections当前大小：" + connections.size());
