...
//		DataExport.exec();
	...
	public static void execNewStockFinance(){
		final Vector<Stock> stocks = StockDAO.getNeedHistoryFinanceDataCodes();
		for (Stock stock : stocks) {
			//logger.info(开始采集：" + stock.getCode());
 				...
 				System.out.println(zyzbjsonArray.length());	
					...
					for(int j=0;j<5;j++){
						...
						if(zyzbjsonArray.length()> j){
                            ...
                            DNNP = StringUtils.GetNunberByUnit(jsonobject.getString("kfjlr"));
                            DNNPGR = StringUtils.GetNunberByUnit(jsonobject.getString("kfjlrtbzz"));
                            if(DNNP == null || DNNP.equals("--")){
                            	DNNP = "0";
                            }
                            if(DNNPGR == null || DNNPGR.equals("--")){
                            	DNNPGR = "0";
                            }
                            finance.setDNNP(DNNP);
                            finance.setDNNPGR(DNNPGR);
						}
						...
						*/
//						StockDAO.updateDNNPAndDNNPGR(finance);
