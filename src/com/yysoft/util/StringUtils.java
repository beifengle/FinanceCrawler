...
public class StringUtils {
	...
	/**
	 * Description:通过正则表达式获取指定第一个字符内容 ，如：getStrByReg("<theme>(.*?)</theme>", str)
	 * @param pattern 正则表达式
	 * @param srcStr 字符源
	 * @return
	 */
	public static String getStrByReg(String pattern, String srcStr) {
	...
	/**
	 * Description:字符不为空且不为null则返回true
	 * @param str
	 * @return
	 */
	...
	public static double getDouble6(String number){
		 DecimalFormat    df6   = new DecimalFormat("######0.000000");  //保留6位小数 
		if(StringUtils.isEmpty(number)){
			return 0;
		}
		
		return Double.valueOf(df6.format(Double.valueOf(number)));
	}
	
	/**
	 * Description:输入财报报告期，返回季度序号
	 * 
	 * @param reportadate 财报报告期
	 * @return
	 */
	public static String getQuarterFromReport(String reportadate) {
		String q = "";
	...
	public static String GetNunberByUnit(String NumUnit){
		DecimalFormat df = new DecimalFormat("######0.0000");

		String Num=NumUnit;
		Double dbnul;
		if (NumUnit.contains("万亿")) {
			dbnul = Double.parseDouble(NumUnit.replace("万亿",""))*100000000*10000;
			Num = df.format(dbnul);
		} else if (NumUnit.contains("亿")) {
			dbnul = Double.parseDouble(NumUnit.replace("亿",""))*100000000;
			Num = df.format(dbnul);
		} else if (NumUnit.contains("百万")) {
			dbnul = Double.parseDouble(NumUnit.replace("百万",""))*1000000;
			Num = df.format(dbnul);
		} else if (NumUnit.contains("万")) {
			dbnul = Double.parseDouble(NumUnit.replace("万",""))*10000;
			Num = df.format(dbnul);
