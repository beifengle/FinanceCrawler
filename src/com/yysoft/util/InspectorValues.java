...
public class InspectorValues {
	...
	public static void check(Finance finance){
		...
		//1、资产负债表内恰关系检查
		 ...
		 if(StringUtils.isNotEmpty(finance.getASSETS())){
			 ASSETS =Double.valueOf(finance.getASSETS().replace("万", ""));//资产总计
		 }
		 if(StringUtils.isNotEmpty(finance.getLIABILITIES())){
			 LIABILITIES =Double.valueOf(finance.getLIABILITIES().replace("万", ""));//负债合计
		 }
		 if(StringUtils.isNotEmpty(finance.getOwnersEquity())){
			 OwnersEquity =Double.valueOf(finance.getOwnersEquity().replace("万", ""));//所有者权益合计
		 }
	...
	/**
	 * 功能描述：采集结果的三个数据源做对比，两两对比
	 */
	public static double selectBestValue(double v1,double v2,double v3){
		double a1 = Math.abs(v1-v2)/v1;
		if (a1>0.1){
			double a2 = Math.abs(v1-v3)/v1;
			if (a2>0.1){
				 double a3 = Math.abs(v2-v3)/v2;
				 if (a3>0.1){
					 return v1;
				 }
				return v2;
	...
	public static void main(String[] args) {
		double b = selectBestValue(1,2,3);
		System.err.println(b);
		b = selectBestValue(1,1.05,3);
		System.err.println(b);
		b = selectBestValue(4,2,2.04);
		System.err.println(b);
		b = selectBestValue(3.2,2,3);
		System.err.println(b);
	}
}
